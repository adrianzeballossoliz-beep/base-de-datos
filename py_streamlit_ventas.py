...
import datetime # Importado para manejo seguro de fechas

# Configuración inicial del dashboard
st.set_page_config(page_title="Hotel – Dashboard General", layout="wide")

# Estilos CSS personalizados (Tema Beige/Café)
st.markdown("""
<style>
...
# CORRECCIÓN CLAVE 1: Cadena de conexión usando el puerto 3307
...
def get_engine(db_uri: str = DB_URI):
    try:
        ...
        # Mensaje útil para verificar el servidor XAMPP y el puerto 3307
        st.error(f"Error conectando a la base de datos. Verifique MySQL (Puerto 3307) y credenciales:\n{e}")
        st.stop()
...
def load_data(db_uri: str):
    ...
    # Consulta SQL completa para unir todas las tablas
    query = """
        SELECT 
            r.id_reserva,
            r.fecha_reserva,
        ...
        LEFT JOIN detalle_pago dp
            ON p.id_pago = dp.id_pago # Corrección de la última revisión
...
# CORRECCIÓN CLAVE 2: Manejo robusto de NaT para definir las fechas mínimas/máximas del filtro
fechas_validas = df["fecha_reserva"].dropna()
if not fechas_validas.empty:
    fecha_min = fechas_validas.min().date()
    fecha_max = fechas_validas.max().date()
else:
    # Fallback si no hay fechas válidas: usar hoy
...
# Se utiliza .drop_duplicates("id_reserva") para contar reservas únicas (resolviendo la duda de 600 vs 300 líneas)
...
# La tabla mostrará todas las filas si las reservas tienen múltiples detalles
